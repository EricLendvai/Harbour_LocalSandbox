...
#include "Harbour_LocalSandbox.ch"
...
#define DOCKER_TABLE_FOLDER "/opt/Harbour_LocalSandbox_Data/"
//=================================================================================================================
Function Main()
...
if l_lInDocker
    oFcgi:PathData := DOCKER_TABLE_FOLDER
endif
...
l_cListOfIncludes := hb_MemoRead("include_paths.txt")
SendToDebugView("test code v2",l_cListOfIncludes)
if empty(l_cListOfIncludes)
    SendToDebugView("Missing include_paths.txt in the backend folder.")
else
    l_aPaths := hb_ATokens(l_cListOfIncludes,.t.)
    for each l_cPath in l_aPaths
        l_cPath := alltrim(l_cPath)
        if !empty(l_cPath)
            if hb_DirExists(l_cPath)
                __pp_path( v_hPP, l_cPath )
                SendToDebugView("Path To Include:",">"+l_cPath+"<")
            endif
        endif
    endfor
endif
...
SendToDebugView("Request Counter v2",::RequestCount)
